...
max_words=201  #seq-len
embed_dim=32    
num_labels=1      
    

def tranformer(top_words=top_words,max_words=max_words,num_labels=num_labels,hidden_dim=[64],num_heads=4,dropout=0.5):
    ...
    x = TransformerEncoder(embed_dim, 32, num_heads=num_heads)(x)
...
def fivecv(max_words=max_words,batch_size=256,epochs=50,hidden_dim=[32],show_loss=True,show_confusion_matrix=True):
    x_train, y, x_test, y_test = F5c_1234(shuffle_data=False)
    x2_train, y2_train, x2_test, y2_test = F5c_encoder(condon)
    x3_train, y3_train, x3_test, y3_test = F5c_encoder(hash1)
    X = np.concatenate([x_train, x3_train], axis=1)
    ...
    for i in range(1,num_folds+1):
        ...
        if   i == 1:
            ...
            auc_plt = auc.train_auc_plot_end(auc_plt = auc_plt,path_name="FB")
        else:
