...
show_loss=True
np.random.seed(0)  
top_words=5   #feature number
max_words=101  #seq-len
embed_dim=32    
num_labels=1      
    

def tranformer(top_words=top_words,max_words=max_words,num_labels=num_labels,hidden_dim=[64],num_heads=16,dropout=0.2):
...
def fivecv(max_words=max_words,batch_size=256,epochs=50,hidden_dim=[32],show_loss=True,show_confusion_matrix=True):
    ...
    for i in range(1,num_folds+1):
        ...
        if   i == 1:
            ...
            auc_plt = auc.train_auc_plot_end(auc_plt = auc_plt,path_name="1234")
        else:
