...
max_words=105  #seq-len
embed_dim=32    
num_labels=1      
    

def tranformer(top_words=top_words,max_words=max_words,num_labels=num_labels,hidden_dim=[64],num_heads=8,dropout=0.5):
    ...
    x = TransformerEncoder(embed_dim, 32, num_heads)(x)
...
def fivecv(max_words=max_words,batch_size=256,epochs=50,hidden_dim=[32],show_loss=True,show_confusion_matrix=True):
    ...
    X = np.concatenate([x3_train, x2_train], axis=1)
    ...
    for i in range(1,num_folds+1):
        ...
        if   i == 1:
            ...
            auc_plt = auc.train_auc_plot_end(auc_plt = auc_plt,path_name="FC")
        else:
